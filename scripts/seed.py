...
async def seed():
    async with async_session_factory() as s:
        ...
        if not acc_id:
            acc_id = (
                await s.execute(
                    text("SELECT id FROM accounts WHERE user_id=:user_id AND exchange='paper'"), {"user_id": user_id}
                )
            ).scalar()

        # Create portfolio (there is no unique constraint so check first)
        port_id = (
            await s.execute(text("SELECT id FROM portfolio WHERE account_id=:account_id"), {"account_id": acc_id})
        ).scalar()
        if not port_id:
            port_res = await s.execute(
                text("""
