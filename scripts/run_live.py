...
async def main():
    ...
    # Resolve dependencies for Supervisor (same as run_backtest)
    ...
    registry.register("execution", agent=ExecutionAgent(engine=engine, session_factory=async_session_factory))
    ...
    while True:
        try:
            ...
            if last_candle_timestamp is None or current_timestamp > last_candle_timestamp:
                ...
                async for state in pipeline.stream_analysis(request=latest_candle, context_params=mem_params):
                    pass
