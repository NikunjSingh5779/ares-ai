...
class BinanceConnector(ExchangeConnector):
    ...
    Configuration keys (passed via config dict)::
        ...
        options     — dict passed to ccxt.binance() options

    Usage::

        conn = BinanceConnector({"api_key": "...", "secret": "..."})
        await conn.connect()
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        if not HAS_CCXT:
            msg = "ccxt is required for Binance connector. Install with: pip install ccxt"
    ...
    async def create_order(
        ...
        # Normalise symbol to CCXT format (e.g. "BTC-USD" → "BTC/USDT")
    ...
    def _require_connected(self) -> None:
        if not self.is_connected:
            raise ExchangeConnectionError("Binance connector is not connected")
    ...
    def _to_ccxt_symbol(symbol: str) -> str:
        ...
        "ETH-USD" → "ETH/USDT"
        "SOL-USD" → "SOL/USDT"
        "BTCUSDT" → "BTC/USDT" (already in CCXT-like format but missing slash)
        """
