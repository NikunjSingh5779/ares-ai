...
class KrakenConnector(ExchangeConnector):
    ...
    async def connect(self) -> bool:
        ...
        try:
            self._client = ccxt.kraken(options)
            ...
            if api_key and secret:
                ...
                if balance.get("free") is None:
                    raise ExchangeConnectionError("Kraken authentication failed — check API key and secret")

            self._connected = True
            return True

        except ccxt.AuthenticationError as exc:
            raise ExchangeConnectionError(f"Kraken authentication failed: {exc}") from exc
        except ccxt.NetworkError as exc:
            raise ExchangeConnectionError(f"Kraken network error: {exc}") from exc
        except Exception as exc:
            raise ExchangeConnectionError(f"Kraken connection failed: {exc}") from exc
    ...
    async def create_order(
        ...
        try:
            ...
            raise OrderRejectedError(f"Order rejected by Kraken: {exc}") from exc
        except ccxt.NetworkError as exc:
